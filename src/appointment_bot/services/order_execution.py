...
def run_rapid_queue_with_settings(
    ...
    *,
    ...
    on_order_start: Callable[[ServiceOrderRuntime], None] | None = None,
...
def run_service_order(
    ...
    *,
    ...
    current_order = get_service_order_runtime(order.order_id, settings=settings)
    if current_order is None or current_order.status != "ready":
        return RunReport(
            status="skipped",
            message="La orden dejo de estar activa antes de iniciar la revision.",
            exit_code=0,
            order_id=order.order_id,
        )
    ...
    def handle_check(*args) -> None:
        if not renew_service_order_claim(
            order.order_id,
            owner_token=lease_owner,
            lease_seconds=SERVICE_ORDER_LEASE_SECONDS,
            settings=settings,
        ):
            raise RuntimeError("The service order lease was lost during monitoring.")
    ...
    report = run_with_report(
        ...
        can_submit=lambda: order_can_submit(order.order_id, lease_owner, settings),
        is_allowed_appointment=_appointment_filter_for_order(order.order_id, settings),
        on_submission_intent=lambda: mark_order_submission_intent(
            order.order_id,
            settings=settings,
        ),
        on_submission_started=lambda: mark_order_submission_pending(
            order.order_id,
            settings=settings,
        ),
    )
    if pending_submission and not report_is_terminal_stage(report):
