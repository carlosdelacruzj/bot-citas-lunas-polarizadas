...
from appointment_bot.worker.post_reservation_review import replace_reports_with_reviewed_evidence
...
class DeferredOrderReports:
    ...
    def flush(self) -> None:
        ...
        summary = RunReport(
            status="completed", message="Evidencias diferidas del monitoreo.", exit_code=0
        )
        notify_deferred_queue_summary(summary, reports, telegram_settings=self.telegram_settings)
        for order_id in sorted(reviewed_order_ids):
            enqueue_whatsapp_automation_job(
                order_id, "reservation_album", settings=self.runtime_settings
            )

    def replace_reviewed_evidence(self, review_results: list[dict[str, str]]) -> None:
        self._reports = replace_reports_with_reviewed_evidence(self._reports, review_results)
        report_order_ids = {report.order_id for report in self._reports if report.order_id}
        self._reviewed_order_ids.update(
            result["order_id"]
            for result in review_results
            if result.get("status") == "completed" and result.get("order_id") in report_order_ids
        )
